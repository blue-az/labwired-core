...
//! STM32F411 Tier-1 fixture firmware (Cortex-M4F, thumbv7em-none-eabi).
//! Targets the STM32F411CEU6 (WeAct "Black Pill"): 512 KiB flash, 128 KiB SRAM.
//!
//! Validates the simulator's chip model peripheral-by-peripheral with RAW
//! REGISTER accesses against the peripherals wired by
//! `configs/chips/stm32f411ceu6.yaml`, reporting one line per peripheral class
...
//!
//! `dma` and `irq` are NOT reported: the F411 yaml declares no DMA/NVIC-class
...
//!
//! The `spi` class covers BOTH SPI1 and **SPI5** — SPI5 is the only instance
//! the F411 has that the F401 does not, and the TIER1 vocabulary has a single
//! `spi` column, so the check exercises the new instance in the same class
//! rather than inventing a column. SPI5's base (0x4001_5000) and IRQ (85) agree
//! between the vendored SVD and `stm32f411xe.h`, but its clock gate
//! (RCC_APB2ENR bit 20) is header-only — no public F411 SVD declares an
//! `SPI5EN` field at all, so this fixture is the only executable check on it.
//!
...
//! the `stm32v2` MODER/ODR/BSRR layout per RM0383) and
...
// ── Wired peripherals (configs/chips/stm32f411ceu6.yaml) ──────────────────
...
const SPI5_BASE: u32 = 0x4001_5000; // F411-only instance; gate APB2ENR.SPI5EN (bit 20)
...
/// One classic-SPI instance: while its APB2 gate is OFF a CR1 write is dropped
/// (reads 0). After enabling, TXE (SR bit 1) is set out of reset; with
/// SPE|MSTR|SSM|SSI a DR write kicks a transfer: BSY (SR bit 7) latches and the
/// cycle-counted engine clears it / re-asserts TXE.
fn check_spi_instance(
    base: u32,
    gate_bit: u32,
    codes: [&'static [u8]; 4],
) -> Result<(), &'static [u8]> {
    wr32(base, 1 << 6); // SPE while unclocked → dropped
    if rd32(base) != 0 {
        return Err(codes[0]);
    }
    wr32(RCC_APB2ENR, rd32(RCC_APB2ENR) | (1 << gate_bit));
    if rd32(base + 0x08) & (1 << 1) == 0 {
        return Err(codes[1]); // SR.TXE @ 0x08
    }
    wr32(base, (1 << 6) | (1 << 2) | (1 << 9) | (1 << 8)); // CR1: SPE|MSTR|SSM|SSI
    unsafe { write_volatile((base + 0x0C) as *mut u8, 0xAB) }; // DR @ 0x0C → start transfer
    if rd32(base + 0x08) & (1 << 7) == 0 {
        return Err(codes[2]); // BSY must be high while the frame shifts
    }
    let mut done = false;
    for _ in 0..20_000 {
        let sr = rd32(base + 0x08);
...
/// spi: SPI1 (gate RCC_APB2ENR.SPI1EN, bit 12) AND **SPI5** (bit 20) — the one
/// instance the F411 adds over the F401. SPI5's gate bit comes from
/// `stm32f411xe.h`; no public F411 SVD declares `SPI5EN` at all, so this check
/// is the only executable evidence that bit 20 gates the right window.
