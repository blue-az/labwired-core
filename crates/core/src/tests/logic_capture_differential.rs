...
mod logic_capture_differential_tests {
    ...
    fn run_bitbang(mut machine: Machine<CortexM>, force_poll: bool, steps: u32) -> RunResult {
        ...
        let idx = machine
            ...
            .unwrap();
        let initial = machine.logic_watch(&[Some(LogicSource::pad(idx, 1)), Some(LogicSource::pad(idx, 0))]);
    ...
    fn stm32_input_pad_push_stream_is_byte_identical_to_poll() {
        let run = |force_poll: bool| {
            ...
            let idx = machine
                ...
                .find_peripheral_index_by_name("gpio_test")
                .unwrap();
            machine.logic_watch(&[Some(LogicSource::pad(idx, 3)), Some(LogicSource::pad(idx, 0))]);
            for slice in 0..6 {
                let level = slice % 2 == 0;
    ...
    fn run_c3_i2c(tick_interval: u32, force_poll: bool) -> Vec<LogicEdge> {
        ...
        let initial = machine.logic_watch(&[Some(LogicSource::pad(gpio_idx, SDA_PIN)), Some(LogicSource::pad(gpio_idx, SCL_PIN))]);
    ...
    fn non_instrumented_peripheral_falls_back_to_poll() {
        ...
        let run = |force_poll: bool| {
            ...
            let poll_idx = machine
                ...
                .unwrap();
            machine.logic_watch(&[Some(LogicSource::pad(push_idx, 0)), Some(LogicSource::pad(poll_idx, 0))]);
            machine.reset_step_profile();
