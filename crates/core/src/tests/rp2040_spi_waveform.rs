...
mod rp2040_spi_waveform_tests {
    ...
    fn every_pad_in_the_table_carries_its_own_instance_and_no_other() {
        ...
        for &(pin, instance) in SPI_PADS {
            ...
            machine.logic_watch(&[Some(LogicSource::pad(sio, pin)), Some(LogicSource::pad(sio, MOSI_PIN))]);
            for _ in 0..20_000 {
                machine.step().unwrap();
            }
            // spi0 alone shifts.
    ...
    fn a_pad_carrying_a_signal_nothing_drives_shows_no_traffic() {
        ...
        machine.logic_watch(&[Some(LogicSource::pad(sio, MOSI_PIN)), Some(LogicSource::pad(sio, NON_DRIVEN_PIN))]);
        for _ in 0..20_000 {
            machine.step().unwrap();
        }
        shift(&mut machine, SPI0_BASE, &[0xA5]);
