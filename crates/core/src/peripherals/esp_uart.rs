...
//! Espressif UART controller — full digital twin, shared by the SoC families
//! that carry this IP.
//!
//! Chip-neutral on purpose: the ESP32-S3 (UART0/1/2) and the ESP32-C3
//! (UART0/1) expose the identical register map, the identical `uart_ll.h`
//! interrupt bits and the same 128-entry FIFOs, so both wire this one model
//! rather than a per-chip copy — see `peripherals::esp32s3::uart` and
//! `peripherals::esp32c3::uart` for each family's source ids and core clock.
//! Anything genuinely per-family (interrupt-matrix source, CPU clock, whether
//! the instance is the stdout console) is a constructor argument, never a
//! branch in here.
//!
...
//! level bits, W1C INT_CLR). Register layout (ESP32-S3 TRM §26 / ESP32-C3 TRM
//! §UART, `soc/uart_reg.h`), at `DR_REG_UART{,1,2}_BASE` = `0x6000_0000` /
//! `0x6001_0000` / `0x6002_E000` (S3; the C3 has the first two):
//!
//! | offset | reg     | behavior                                              |
//! |--------|---------|-------------------------------------------------------|
...
//!
//! `tick()` emits the UART interrupt-matrix source the instance was built with
//! (S3: UART0=27/1=28/2=29; C3: UART0=21/1=22) while `INT_ST != 0`; the bus
//! routes it through the per-core interrupt matrix like the systimer tick.
//! Self-contained type, distinct from the STM32 `Uart` — mapping an Espressif
//! UART onto that STM32 layout puts the FIFO write on a status register and
//! drops the TX interrupt entirely, which deadlocks ESP-IDF's blocking
//! `uart_tx_all()` on any write longer than the FIFO.
//!
...
/// Hardware TX/RX FIFO depth (`SOC_UART_FIFO_LEN`). Same on the ESP32-S3 and
/// the ESP32-C3 — both carry this UART IP.
const FIFO_LEN: usize = 128;
/// UART source clock (APB) — scale baud timing into ticks.
const UART_SCLK_HZ: u64 = 80_000_000;
/// Default CPU/tick clock (ESP32-S3). The ESP32-C3 runs its core at 160 MHz, so
...
pub struct EspUart {
    ...
    scheduled: bool,
    /// Core clock this instance is paced against (S3 = 240 MHz, C3 = 160 MHz).
    /// 0 only via `Default`, which no bus path uses; `cycles_per_byte` clamps.
    cpu_clock_hz: u64,
    /// Externally-fed RX queue, shared with `attach_uart_rx_source[_named]`.
    /// Bytes pushed here are moved into the RX FIFO on the next access — the
...
impl EspUart {
    ...
    pub fn new(echo_stdout: bool, source_id: u32) -> Self {
        Self::new_with_cpu_clock(echo_stdout, source_id, CPU_CLOCK_HZ)
    }

    /// Same as [`EspUart::new`], but paced against an explicit core clock.
    ...
    /// moved. Called before every register access (reads take `&self`, hence
    ...
    /// `ingest_rx_source` plus the `RXFIFO_TOUT` latch that [`Self::push_rx`]
    /// sets — the tick path can take `&mut self`, so it can raise the edge bit.
    ...
    /// output (the browser bridge, or the CLI's `--no-uart-stdout`).
    ///
    /// Echo can only be turned OFF here, never on: which instance is the
    /// console is decided at construction (UART0 yes, UART1/UART2 no), so a
    /// run-level "echo" flag must not promote a quiet instance into one.
    pub fn silence_stdout_echo_if(&mut self, echo_stdout: bool) {
    ...
    fn cycles_per_byte(&self) -> u64 {
        ...
        (10 * clkdiv * self.cpu_clock_hz / UART_SCLK_HZ).max(1)
    }
...
mod tests {
    ...
    fn tx_fifo_fills_then_drains_to_sink() {
        ...
        let mut u = EspUart::new(false, 27);
    ...
    fn rx_read_pops_and_consumes() {
        let mut u = EspUart::new(false, 28);
        u.push_rx(b'A');
        u.push_rx(b'B');
